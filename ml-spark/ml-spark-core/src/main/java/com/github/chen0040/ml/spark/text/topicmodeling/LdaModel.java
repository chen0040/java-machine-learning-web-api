 ...
 */
public class LdaModel implements Serializable, Cloneable {
    ...
    private void copy(LdaModel rhs){
        vocabulary = (Vocabulary)((BasicVocabulary)rhs.vocabulary).clone();
        topicCount = rhs.topicCount;
        wordTopicCounts = rhs.wordTopicCounts.clone();
        topicWordCounts = new List[rhs.topicWordCounts.length];
    ...
    public double entropy(int[] counts) {
        ...
        return Math.log(sum) - (1.0 / sum) * sum(counts, x ->  x * Math.log(x));
    }

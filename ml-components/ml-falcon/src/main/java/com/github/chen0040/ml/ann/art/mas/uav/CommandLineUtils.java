...
public class CommandLineUtils {
    public static UavSimulator procCommandLines(String[] args, UavSimulatorConfig simConfig, FalconConfig aiConfig){
        ...
        simConfig.setMaxTrial(100);
        simConfig.setInterval(10);

        simConfig.loadUav_a280();

        if(!simConfig.applyTDFALCON) {
            return new UavSimulatorAS(simConfig, aiConfig);
        }

        aiConfig.numAction = simConfig.numActions;
        aiConfig.numState = simConfig.numStates;
        aiConfig.numReward = 2;


        if(falconMode.equalsIgnoreCase("sarsa")) {
            return new UavSimulatorSarsa(simConfig, aiConfig);
        } else if(falconMode.equalsIgnoreCase("qlearn")) {
            return new UavSimulatorSarsa(simConfig, aiConfig);
        } else if(falconMode.equalsIgnoreCase("sarsalambda")) {
            return new UavSimulatorSarsaLambda(simConfig, aiConfig);
        } else if(falconMode.equalsIgnoreCase("r")) {
            return new UavSimulatorR(simConfig, aiConfig);
        } else {
            return new UavSimulatorQLambda(simConfig, aiConfig);
        }
    }
}

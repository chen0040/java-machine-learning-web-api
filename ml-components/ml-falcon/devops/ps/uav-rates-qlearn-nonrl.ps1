...
$bounded=0
$simCount=6
$folderName="tdfalcon/uav-nonrl-q"
...
for($i=0; $i -lt 30; $i+=$simCount){
    ...
    Start-Job -ScriptBlock { Param($one,$two,$three,$four,$five,$six,$seven,$eight,$nine, $ten, $eleven)
     java -jar $HOME/tdfalcon/tdfalcon-0.0.1.jar -j URAV -s $one -c $two -t $three -m $four -f $five -b $six -h $seven -g $eight -n $nine > $HOME/tdfalcon/console/console.out-$ten 2> $HOME/tdfalcon/console/console.error-$eleven
    } -ArgumentList $sparseDegree,$numAgents,$targetMove,$falconMode,$rl,$bounded,$simIndex,$simCount,$folderName,$simIndex,$simIndex

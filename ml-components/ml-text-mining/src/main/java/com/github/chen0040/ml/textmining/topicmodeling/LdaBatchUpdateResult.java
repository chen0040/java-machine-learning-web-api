...
public class LdaBatchUpdateResult extends BatchUpdateResult {
    ...
    public List<Tuple2<Double, Doc>> scoreDocuments(int selectedTopicIndex){
    ...
    public List<Tuple2<Double, Doc>> reorderDocuments(int selectedTopicIndex) {

        if(selectedTopicIndex==-1) return null;

        List<Tuple2<Double, Doc>> result = scoreDocuments(selectedTopicIndex);
        result.sort((s1, s2)->{
            double f1 = s1._1();
            double f2 = s2._1();

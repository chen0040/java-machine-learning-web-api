...
public class svm_model implements Cloneable, java.io.Serializable {
	...
	public void copy(svm_model rhs){
		...
		l = rhs.l;            // total #SV
		SV=new svm_node[rhs.SV.length][];    // SVs (SV[l])

		for(int i=0; i < rhs.SV.length; ++i){
			SV[i] = new svm_node[rhs.SV[i].length];
			for(int j=0; j < rhs.SV[i].length; ++j){
				SV[i][j] = (svm_node)rhs.SV[i][j].clone();
			}
		}
		...
		sv_indices = rhs.sv_indices == null ? null : rhs.sv_indices.clone();       // sv_indices[0,...,nSV-1] are values in [1,...,num_traning_data] to indicate SVs in the training set
